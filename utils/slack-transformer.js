...
module.exports.formatMessage = message => {
  ...
  return {
    ...
    userId: message.user ? message.user : (message && message.message && message.message.user),
    channel: message.channel,
    channel_name: cache.allChannels[message.channel],
    text: message.text ? message.text : (message && message.message && message.message.text),
    team: message.team,
    event_type: message.thread_ts ? 'message_replied' : (message.subtype || message.type),
    ...
    reply_users_count: message && message.message && message.message.reply_users_count,
    replies: message && message.message && message.message.replies,
    latest_reply: message && message.message && message.message.latest_reply,
    reactions: message.reactions || (message && message.message && message.message.reactions),
    pinned_to: message.pinned_to || (message && message.message && message.message.pinned_to)
  }
}

module.exports.formatReaction = ({
  event_type = 'reaction_added',
  reaction, channel, user, team, text, type, event_ts, item: { ts: message_ts }
}) => {
  return {
    reaction,
    channel,
